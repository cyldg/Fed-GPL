...
def prepare_data(data_base_path, batch, min_data_ratio=0.3):
...
def prepare_data_split(data_base_path, batch, min_data_ratio=0.3,alpha =0.01):
    transform_train = transforms.Compose([
            transforms.Resize([224, 224]),            
            transforms.RandomHorizontalFlip(),
            # transforms.RandomRotation((-30,30)),
    ...
    sketch_test_loader = torch.utils.data.DataLoader(sketch_testset, batch_size=batch, shuffle=False)
    #split train_loader
    # clipart_net_dataidx_map = partition_domainnet_dataset(clipart_trainset, 5,alpha)
    # print(len(clipart_net_dataidx_map[0]))
    # infograph_net_dataidx_map = partition_domainnet_dataset(infograph_trainset,5,alpha)
    # painting_net_dataidx_map = partition_domainnet_dataset(painting_trainset, 5,alpha)
    # quickdraw_net_dataidx_map = partition_domainnet_dataset(quickdraw_trainset, 5,alpha)
    # real_net_dataidx_map = partition_domainnet_dataset(real_trainset, 5,alpha)
    # sketch_net_dataidx_map = partition_domainnet_dataset(sketch_trainset, 5,alpha)
    train_loader = []
    clipart_net_dataidx_map = partition( alpha,5,np.array([label for _, label in clipart_trainset]),10)
    print("length",len(clipart_net_dataidx_map[0]))
    for idx_batch in clipart_net_dataidx_map.values():
        subset_dataset = torch.utils.data.Subset(clipart_trainset, idx_batch)
        subset_dataset = torch.utils.data.DataLoader(subset_dataset, batch_size=batch, shuffle=True)
        train_loader.append(subset_dataset)

    infograph_net_dataidx_map = partition(alpha,5,np.array([label for _, label in infograph_trainset]),10)
    for idx_batch in infograph_net_dataidx_map.values():
        subset_dataset = torch.utils.data.Subset(infograph_trainset, idx_batch)
        subset_dataset = torch.utils.data.DataLoader(subset_dataset, batch_size=batch, shuffle=True)
        train_loader.append(subset_dataset)

    painting_net_dataidx_map = partition(alpha,5,np.array([label for _, label in painting_trainset]),10)
    for idx_batch in painting_net_dataidx_map.values():
        subset_dataset = torch.utils.data.Subset(painting_trainset, idx_batch)
        subset_dataset = torch.utils.data.DataLoader(subset_dataset, batch_size=batch, shuffle=True)
        train_loader.append(subset_dataset)

    quickdraw_net_dataidx_map = partition(alpha,5,np.array([label for _, label in quickdraw_trainset]),10)
    for idx_batch in quickdraw_net_dataidx_map.values():
        subset_dataset = torch.utils.data.Subset(quickdraw_trainset, idx_batch)
        subset_dataset = torch.utils.data.DataLoader(subset_dataset, batch_size=batch, shuffle=True)
        train_loader.append(subset_dataset)

    real_net_dataidx_map = partition(alpha,5,np.array([label for _, label in real_trainset]),10)
    for idx_batch in real_net_dataidx_map.values():
        subset_dataset = torch.utils.data.Subset(real_trainset, idx_batch)
        subset_dataset = torch.utils.data.DataLoader(subset_dataset, batch_size=batch, shuffle=True)
        train_loader.append(subset_dataset)

    sketch_net_dataidx_map = partition(alpha,5,np.array([label for _, label in sketch_trainset]),10)
    for idx_batch in sketch_net_dataidx_map.values():
        subset_dataset = torch.utils.data.Subset(sketch_trainset, idx_batch)
        subset_dataset = torch.utils.data.DataLoader(subset_dataset, batch_size=batch, shuffle=True)
        train_loader.append(subset_dataset)
...
def partition(alphaa, n_netss, y_train,classes):
    min_size = 0
    n_nets = n_netss
    N = len(y_train)
    #print(N)
