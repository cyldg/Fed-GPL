...
output_folder = os.path.join("DR_GPL",datetime.now().strftime("%m%d%Y_%H%M%S"),"DR")
...
if __name__ == "__main__":
    ...
    logout = f"fedgen_dr_{args.prompt}_{args.data_len}"
    ...
    server_model = PGVIT(img_size=edge_size, patch_size=patch_size, Prompt_Token_num=Prompt_Token_num,
                    VPT_type=VPT_type)

    server_model.load_state_dict(basic_model.state_dict(), False)
    server_model.New_CLS_head(num_classes)
    server_model.Freeze()
    ...
    for rounds in range(args.iterations):
        ...
        for i, model in enumerate(client_models):
            for  iters in range(args.epoches):
                ...
                print(f'Train Epoch:{rounds} | {domain[i]} | Train iters: {iters} | Train Loss: {train_loss:.{4}f} | Train Acc: {train_acc:.{4}f}')
        ...
        for client_idx, datasite in enumerate(domain):
            ...
            if logout:
                with open(f'{logout}.txt', 'a+') as f_out:
                    f_out.write(f'Test Epoch:{rounds} | {domain[client_idx]} | Test Acc: {np.mean(test_acc):.{4}f} \n')   
        for client_idx, datasite in enumerate(domain):
            print("savemodel")
