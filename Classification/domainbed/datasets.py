...
class ReshapeAndTranspose(object):
    def __call__(self, img):
        # 将（3 x 224 x 224）的数据转换为（224 x 224 x 3）的数据
        img = img.permute(1, 2, 0)  # 将通道维度置于最后
        return img
...
class Latent_DR_Dataset(Dataset):
    ...
    def load_dataset(self):
        ...
        classes, class_to_idx = self.find_classes(self.root_dir + self.domain + '/')
        self.num_class = len(classes)
        messidor_idx={"0":0,"1":1,"2":2,"3":3}

        path = self.root_dir + self.domain + '/'
        if "Messidor-1" in path:
            print(path,class_to_idx,IMG_EXTENSIONS)
...
class Latent_DR_Dataset_mix(Dataset):
    def __init__(self, root_dir, domain, split, get_domain_label=False, get_cluster=False, color_jitter=True,
                 min_scale=0.8):
        self.root_dir = root_dir
        self.domain = domain
    ...
    def load_dataset(self):
        total_samples = []
        self.domains = np.zeros(0)
        mix_data_path = '/home/yuliang_chen/shared_data'
        classes, class_to_idx = self.find_classes(self.root_dir + self.domain + '/')
        classes2, class_to_idx2 = self.find_classes('/home/yuliang_chen/shared_data' + '/')
        self.num_class = len(classes)
        path = self.root_dir + self.domain + '/'
        print(path,class_to_idx,IMG_EXTENSIONS)
        samples = make_dataset(path, class_to_idx, IMG_EXTENSIONS)
        ...
        sample2 = make_dataset(mix_data_path,class_to_idx2,IMG_EXTENSIONS)
        print(len(sample2))
        total_samples.extend(sample2)
...
class DG_Dataset_mix(Dataset):
    ...
    def load_dataset(self):
        total_samples = []
        self.domains = np.zeros(0)
        mix_data_path = '/home/yuliang_chen/PLDG/skin_share'
        # classes = ['dog', 'elephant', 'giraffe', 'guitar', 'horse', 'house', 'person']
        #  class_to_idx = {'dog': 0, 'elephant': 1, 'giraffe': 2, 'guitar': 3, 'horse': 4, 'house': 5, 'person': 6}
        classes, class_to_idx = self.find_classes(self.root_dir + self.domain + '/')
        classes2, class_to_idx2 = self.find_classes('/home/yuliang_chen/PLDG/skin_share'+'/')
        self.num_class = len(classes)
        # self.domain=['art_painting', 'cartoon', 'sketch']
