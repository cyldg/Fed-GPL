...
from domainbed.datasets import Latent_DR_Dataset,DG_Dataset
...
from generate import PGVIT,PGVITBdeep
...
output_folder = os.path.join("ISIC_GPL",datetime.now().strftime("%m%d%Y_%H%M%S"),"ISIC")
...
if __name__ == "__main__":
    ...
    logout = f"ISICFED_gendeep_{args.prompt}_{args.data_len}"
    ...
    for i in range(len(domain)):
        temp = DG_Dataset(root_dir=f"{args.data_dir}{args.dataset}/", domain=domain[i], split='train', color_jitter=True)
    ...
    VPT_type="shallow"
    patch_size = 16
    Prompt_Token_num=args.prompt
    basic_model = timm.create_model('vit_base_patch' + str(patch_size) + '_' + str(edge_size),
                                    pretrained=True, pretrained_cfg_overlay = dict(file="pytorch_model.bin"))

    server_model = PGVITBdeep(img_size=edge_size, patch_size=patch_size, Prompt_Token_num=Prompt_Token_num,
                    VPT_type=VPT_type)
    
    server_model.load_state_dict(basic_model.state_dict(), False)
    server_model.New_CLS_head(num_class)
    server_model.Freeze()
    ...
    if logout:
        with open(f'{logout}.txt', 'a+') as f_out:
            f_out.write(f'lr:{lr}|round:{args.iterations}')

    for rounds in range(0,args.iterations):
        optimizers = [optim.AdamW(params=client_models[idx].parameters(),lr = lr,weight_decay=1e-2)  for idx in range(client_num) ]
        for i, model in enumerate(client_models):
            for  iters in range(args.epoches):
                train_loss, train_acc,train_aoc = public_utils.ISIC_vpt_train(model, train_loader[i], optimizers[i], loss_fun, device)
                print(f'Train Epoch:{rounds} | {domain[i]} | Train iters: {iters} | Train Loss: {train_loss:.{4}f} | Train Acc: {train_acc:.{4}f}| train aoc:{train_aoc:.{4}f}')
        ...
        for client_idx, datasite in enumerate(domain):
            print("savemodel")
            dict = client_models[client_idx].obtain_model()
            print(dict.keys())
        ...
        if best_acc < np.mean(test_acc_mean):
            ...
            torch.save(server_model.state_dict(), os.path.join(output_folder, '01042026_Avg_Best.pth'))
